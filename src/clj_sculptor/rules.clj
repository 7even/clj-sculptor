...
(defn- extract-leading-comments
  ...
  [item-maps]
  (let [comments (take-while standalone-comment?
                             item-maps)
        remaining (drop-while standalone-comment?
                              item-maps)]
    [(vec comments)
     (vec remaining)]))
  ...
  (let [final-state (reduce (fn [{:keys [result
                              ...
                              (case (n/tag child)
                                ...
                                (if (and (some? current-element)
                                  ...
                                  (let [element-map (assoc current-element
                                                           :comment
                                                           child)]
                                    (-> state
                                        (update :result
...
(defn- format-pair-comment
  ...
  [node
   ...
   is-last?
   needs-spacing?]
  (let [formatted-comment (format-node item-indent
                                       node)]
    (if (and is-last?
             needs-spacing?)
      [formatted-comment
       (n/newlines 1)
       (n/spaces item-indent)]
      [formatted-comment])))
...
(defn- calculate-value-indent
  ...
  [key-string
   key-indent]
  (let [{:keys [multi-line?
                last-line]} (analyze-multiline key-string)]
    (if multi-line?
      (+ (count last-line)
         1)
      (+ key-indent
...
(defn- format-pair
  ...
  [key-item
   ...
   is-last?
   needs-spacing?]
  (let [format-child (partial format-node
                              item-indent)
        ;; Format the key
        formatted-key (format-child (:element key-item))
        key-string (n/string formatted-key)
        value-indent (calculate-value-indent key-string
                                             item-indent)
...
(defn- format-pair-entries
  ...
  (let [format-child (partial format-node
    ...
    (map-indexed (fn [entry-idx
                      entry]
                   (let [is-last? (= entry-idx
                                     (dec (count pairs)))]
                     (case (:type entry)
                       :comment
  ...
  (let [[symbol-name-map
        ...
        formatted-values (map (partial format-element-with-prefix-and-comment
                                       (+ indent
                                          2))
                              value-maps)
        separator (make-separator (+ indent
                                     2))
        values-with-spacing (interpose separator
                                       formatted-values)]
    (->> values-with-spacing
         (concat [form-type
                  (n/spaces 1)
...
(defn format-defmulti
  ...
  (let [[name-map
        ...
        separator (make-separator (+ indent
                                     2))
        dispatch-with-spacing (interpose separator
                                         formatted-dispatch)]
...
(defn format-defmethod
  ...
  (let [[method-name-map
        ...
        dispatch-str (n/string (format-node (+ indent
                                               1)
                                            dispatch-val))
        ...
        args-indent (+ indent
                       ...
                       (count dispatch-str)
                       1)
        formatted-args (format-args-vector args-vec
                                           args-indent)
        ...
        formatted-body (map (partial format-element-with-prefix-and-comment
                                     (+ indent
                                        2))
                            body-maps)
        body-separator (make-separator (+ indent
                                          2))]
    (->> (concat [form-type
                  (n/spaces 1)
                  method-name
    ...
    (if (= (n/tag bindings-vec)
      ...
      (let [bindings-children (n/children bindings-vec)
            ...
            bindings-indent (+ indent
                               ...
                               1
                               1) ;; paren + form + space + [
            formatted-bindings (->> (format-pair-entries bindings-indent
                                                         bindings-pairs)
...
(defn format-function-form [form-type
  ...
  (let [form-symbol (n/sexpr form-type)
        ...
        [docstring-map
         remaining-maps] (if (and (not is-fn-form?)
                                  (seq remaining-maps)
                                  (-> remaining-maps
                                      first
                                      :element
...
(defn format-conditional-form [form-type
                               args
                               indent]
                        ...
                        (let [elem (:element item-map)]
                          (if (and (= (n/tag elem)
                            ...
                            (let [children (n/children elem)
                                  [clause-type
                                   & clause-args] (->> children
                                                       (remove (comp strippable?
                                                                     n/tag))
                                                       vec)
                                  clause-body-indent (+ body-indent
                                                        2)
        ...
        processed (reduce (fn [{:keys [result
                            ...
                            (cond
                              ...
                              (let [formatted-comment (format-entry current)
                                    ...
                                    comment-with-sep [formatted-comment
                                                      body-separator]]
                                ;; Check if this comment is right after an orphaned key
                                (cond
                                  ;; Don't add blank line before comment after orphaned key
                                  after-orphaned-key?
                                  {:result (conj result
                                                 body-separator
                                                 comment-with-sep)
...
(defn format-pair-based-form [form-type
                              args
                              indent]
    ...
    (case form-type-str
      ...
      (let [[expr-map
            ...
            expr-align (+ indent
                          ...
                          (count form-type-str)
                          1)
            formatted-expr (format-element-with-prefix-and-comment expr-align
                                                                   expr-map)
            processed (process-pair-args-with-comments pair-args
                                                       indent)]
        (->> (concat [form-type
                      (n/spaces 1)
                      formatted-expr
      ...
      (let [[pred-map
            ...
            pred-align (+ indent
                          ...
                          (count form-type-str)
                          1)
            formatted-pred (format-element-with-prefix-and-comment pred-align
                                                                   pred-map)
            pred-string (n/string formatted-pred)
            expr-indent (calculate-value-indent pred-string
                                                pred-align)
            formatted-expr (format-element-with-prefix-and-comment expr-indent
                                                                   expr-map)
            ;; Process pairs in groups of two
            pair-groups (let [pairs (partition-all 2
                                                   pair-args)]
                          (reduce (fn [result
                                       pair]
                                    (if (= (count pair)
            ...
            pairs-content (interpose blank-line-separator
                                     pair-groups)]
        (->> (concat [form-type
                      (n/spaces 1)
                      formatted-pred
      ...
      (let [[expr-map
            ...
            non-comment-args (remove standalone-comment?
                                     pair-args)
            [pairs
             default-val] (if (odd? (count non-comment-args))
                            [(butlast pair-args)
                             (last pair-args)]
                            [pair-args
                             nil])
            processed (process-pair-args-with-comments pairs
                                                       indent)
            formatted-default (when (some? default-val)
                                (format-element-with-prefix-and-comment entries-indent
                                                                        default-val))
            ;; Only add blank line if default exists and the last pair wasn't a comment
            content-with-default (when (some? formatted-default)
                                   ;; Check if we need a blank line before default
                                   ;; If last item in pairs was a comment, no blank line needed
                                   (let [last-pair-item (last pairs)]
                                     (if (and last-pair-item
                                              (standalone-comment? last-pair-item))
                                       [formatted-default]
                                       (concat blank-line-separator
                                               [formatted-default]))))]
        (->> (concat [form-type
                      (n/spaces 1)
                      formatted-expr
      ...
      (let [processed (process-pair-args-with-comments args
                                                       indent)]
        (->> (concat [form-type
                      body-separator]
                     processed)
...
(defn format-body-only-form
  ...
  (if (and (= (count args)
    ...
    (let [comment-node (:element (first args))
          formatted-comment (format-node indent
                                         comment-node)
          body-indent (+ indent
                         2)
          elements [form-type
                    (n/spaces 1)
                    formatted-comment]
          final-elements (add-closing-bracket-spacing elements
                                                      body-indent)]
...
(defn- format-import-list
  ...
  (if (= (n/tag import-node)
    ...
    (let [children (n/children import-node)
          cleaned (remove (comp strippable?
                                n/tag)
                          children)
          [package
           & classes] cleaned
      ...
      (if (seq classes)
        (let [first-class (first classes)
              ...
              class-indent (+ base-indent
                              1
                              (count package-str)
                              1)]
          (n/list-node (concat [package
                                (n/spaces 1)
                                first-class]
                               (mapcat (fn [cls]
                                         [(n/newlines 1)
                            ...
                            (map (fn [item]
                                   (let [elem (:element item)
                                     ...
                                     (assoc item
                                            :element
                                            as-vector))))
                            (sort-by (comp n/string
                                           :element)))
        formatted-first (format-element-with-prefix-and-comment (+ indent
                                                                   3)
                                                                first-elem)]
  ...
  (let [;; Convert all collections to lists and sort alphabetically
    ...
    (if (seq import-lists)
      (let [;; All import lists should align at the same column
            ...
            import-str (-> first-elem
                           :element
                           n/string)
            import-list-indent (+ indent
                                  3
                                  (count import-str)
                                  1)
            ;; Format all import lists with the same base indent
            [first-list
             & rest-lists] import-lists
          ...
          (let [[first-elem
            ...
            (cond
              ...
              (and (is-token? first-node)
                   (= (n/sexpr first-node)
                      :require))
              (format-require-clause first-elem
                                     rest-elems
                                     indent)
              ...
              (and (is-token? first-node)
                   (= (n/sexpr first-node)
                      :import))
              (format-import-clause first-elem
                                    rest-elems
                                    indent)
...
(defn format-ns
  ...
  (let [[ns-name-map
        ...
        {require-clauses :require
         ...
         other-clauses :other} (reduce (fn [acc
                                            clause-map]
                                       ...
                                       clauses)
        ;; Format each group
        formatted-other (map (partial format-ns-clause
                                      indent)
                             other-clauses)
        formatted-requires (map (partial format-ns-clause
                                         indent)
                                require-clauses)
        formatted-imports (map (partial format-ns-clause
                                        indent)
                               import-clauses)
...
(defmethod format-node :comment [indent
                                 node]
...
;; Vector formatting
(defmethod format-node :vector [indent
                                node]
  (let [children (n/children node)
        grouped (group-with-prefixes-and-comments children)]
    (if (empty? grouped)
      ...
      (let [;; Format each element
            ...
            separator (make-separator (+ indent
                                         1))
            elements-with-separators (interpose separator
                                                formatted)
            ;; Add closing bracket spacing if needed
...
;; Map formatting
(defmethod format-node :map [indent
                             node]
  (->> (n/children node)
       group-with-prefixes-and-comments
       build-pairs
       (format-pair-entries (+ indent
                               1))
       (interpose (make-separator (+ indent
                                     1)))
       flatten
       n/map-node))

;; Set formatting
(defmethod format-node :set [indent
                             node]
  (let [children (n/children node)
        grouped (group-with-prefixes-and-comments children)
        formatted (map (partial format-element-with-prefix-and-comment
...
;; Quote formatting
(defmethod format-node :quote [indent
                               node]
  (let [children (n/children node)]
    (if (empty? children)
      node
      (let [inner-expr (first children)
            ...
            formatted-inner (format-node (+ indent
                                            1)
                                         inner-expr)]
        (n/quote-node formatted-inner)))))

;; Syntax-quote formatting
(defmethod format-node :syntax-quote [indent
                                      node]
  (let [children (n/children node)]
    (if (empty? children)
      node
...
(defmethod format-node :unquote [indent
                                 node]
  (let [children (n/children node)]
    (if (empty? children)
      node
...
(defmethod format-node :unquote-splicing [indent
                                          node]
  (let [children (n/children node)]
    (if (empty? children)
      node
...
;; Var formatting (#'x)
(defmethod format-node :var [indent
                             node]
  (let [children (n/children node)]
    (if (empty? children)
      node
...
(defmethod format-node :fn [indent
                            node]
  ...
  (let [list-node (n/list-node (n/children node))
        ...
        formatted-list (format-node (+ indent
                                       1)
                                    list-node)]
    ...
    (let [children (n/children node)
      ...
      (if (and (seq non-ws)
        ...
        (let [[inner-uneval
              ...
              flattened-remaining (map n/uneval-node
                                       remaining-in-outer)]
          ;; Combine all the flattened unevals
          (concat flattened-inner
                  flattened-remaining))
...
(defmethod format-node :uneval [indent
                                node]
  ;; #_ reader macro (comment out form)
  (let [children (n/children node)]
    (if (empty? children)
      node
      (let [non-whitespace (remove #(strippable? (n/tag %))
                                   children)]
        (if (and (seq non-whitespace)
          ...
          (let [flattened (flatten-stacked-unevals node)
                formatted-flattened (map #(format-node indent
                                                       %)
                                         flattened)]
...
(defmethod format-node :forms [indent
                               node]
  (let [children (try
                   (n/children node)
                   (catch Exception _
...
(defn- format-function-call
  ...
  (let [function-string (n/string formatted-function)
        {:keys [multi-line?
                last-line]} (analyze-multiline function-string)
    ...
    (if (empty? formatted-args)
      ...
      (let [elements [formatted-function]
        ...
        (n/list-node (flatten final-elements)))
      ;; Has arguments
      (let [first-line [formatted-function
                        (n/spaces 1)
                        (first formatted-args)]
...
;; List formatting
(defmethod format-node :list [indent
                              node]
  (let [children (n/children node)
        cleaned (group-with-prefixes-and-comments children)]
    (if (empty? cleaned)
      ...
      (let [format-child (partial format-node
                                  indent)
            [first-element
             & rest-elements] cleaned
            ;; Check if first element has inline comment
            [actual-first-element
             inline-comment-after-fn] (if (:comment first-element)
        ...
        (cond
          ...
          (and (seq all-leading-comments)
               (seq actual-args)
               (not (contains? comment-handling-forms
                               function-symbol)))
          (let [formatted-comments (map (partial format-node
                                                 indent)
                                        all-leading-comments)
                ...
                new-list-node (-> (cons actual-first-element
                                        children-after-leading)
                                  n/list-node
                                  format-child)]
          ...
          (let [formatted-comments (map (partial format-node
                ...
                simple-function (format-node (+ indent
                                                1)
                                             actual-first-element)
          ...
          (contains? special-form-handlers
                     function-symbol)
          (let [handler (get special-form-handlers
                             function-symbol)
                ;; For forms that handle comments, include all comments as args
                args-to-pass (if (contains? comment-handling-forms
                                            function-symbol)
                               (concat (when (some? inline-comment-after-fn)
                                       ...
                                       actual-args)
                               actual-args)]
            (handler actual-first-element
                     args-to-pass
                     indent))

          ;; Regular function call - pass the grouped args directly
          :else
          (format-function-call formatted-function
                                actual-args
                                indent))))))
